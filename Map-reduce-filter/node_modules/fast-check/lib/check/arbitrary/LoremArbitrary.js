...
function lorem(maxWordsCount, sentencesMode) {
    ...
    if (sentencesMode) {
        const sentence = ArrayArbitrary_1.array(loremWord(), 1, 10)
            .map((words) => words.join(' '))
            .map((s) => (s[s.length - 1] === ',' ? s.substr(0, s.length - 1) : s))
            .map((s) => s[0].toUpperCase() + s.substring(1) + '.');
        return ArrayArbitrary_1.array(sentence, 1, maxCount).map((sentences) => sentences.join(' '));
    }
    else {
        return ArrayArbitrary_1.array(loremWord(), 1, maxCount).map((words) => words.map((w) => (w[w.length - 1] === ',' ? w.substr(0, w.length - 1) : w)).join(' '));
    }
}
exports.lorem = lorem;
